#!/usr/bin/env python3
"""
Test version of 01-tcr-chains_mesh_phch.py
Directly accepts pdb_file path instead of looking up a file list.

Usage:
    python3 01-test-tcr_mesh_phch.py <pdb_file> <pdb_id> <tcr_chains> [out_dir]

Example:
    python3 01-test-tcr_mesh_phch.py 00-raw_pdbs/1AO7.pdb 1AO7 DE
"""
import os
import sys
...
if len(sys.argv) < 4:
    print("Usage: python3 {} <pdb_file> <pdb_id> <tcr_chains>".format(sys.argv[0]))
    sys.exit(1)
...
# Save surface vertex coords (p2 = TCR side)
save_dir = os.path.join(params["fp_precomputation_dir"], pdb_id)
os.makedirs(save_dir, exist_ok=True)
np.save(os.path.join(save_dir, "p2_X.npy"), regular_mesh.vertices[:, 0])
np.save(os.path.join(save_dir, "p2_Y.npy"), regular_mesh.vertices[:, 1])
np.save(os.path.join(save_dir, "p2_Z.npy"), regular_mesh.vertices[:, 2])

print("TCR surface done. Saved to:", save_dir)
