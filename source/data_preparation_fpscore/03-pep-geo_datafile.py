...
for ppi_pair_id in ppi_pair_list:
    ...
    ply_file = {}
    # pmhc, 25.Dec
    ply_file['p1'] = masif_opts['fp-ply_file_template'].format(
        dir_prefix, fields[2])
    ''' # tcr 
    ...
    p1_npy_path = os.path.join(patch_index_dir, f"{dir_prefix}_p1_idx.npy")
    p1_residue_map = np.load(p1_npy_path, allow_pickle=True).item()
    p1_index = [v for v in p1_residue_map.values() if v is not None]
    print(f"{pdb}: 已读取 {len(p1_index)} 个 pMHC 有效中心点索引。")
    all_center_index['p1'] = p1_index

    '''
    p2_npy_path = os.path.join(patch_index_dir, f"{pdb}_p2_idx.npy")
    p2_residue_map = np.load(p2_npy_path, allow_pickle=True).item()
    p2_index = [v for v in p2_residue_map.values() if v is not None]
    print(f"{pdb}: 已读取 {len(p2_index)} 个 TCR 有效中心点索引。")
    all_center_index['p2'] = p2_index
    '''

    out_ply_file = {}
    out_ply_file['p1'] = ply_set_out_dir + \
        "/p1_{}_{}.ply".format(dir_prefix, fields[2])
    '''
    out_ply_file['p2'] = ply_set_out_dir + \
        "/p2_{}_{}.ply".format(fields[0], fields[2])
    '''
