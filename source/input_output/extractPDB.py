...
def extractPDB(
    ...
    ##### 240712： 80机子上处理docking结构时，提取model行报错，但在本地调试时未发现异常（补充：错误不在这步！）
    #model = Selection.unfold_entities(struct, "M")[0] ## M=Model
    ## 打印结构信息
    print(f"Structure ID: {struct.id}")
    ## 240712：尝试改为下两行命令
    model = Selection.unfold_entities(struct, "M")[0]
    print(f"Number of models: {len(model)}")


    chains = Selection.unfold_entities(struct, "C") ## C=Chain
